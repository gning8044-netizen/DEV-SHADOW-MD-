...
async function muteCommand(sock, chatId, senderId, message, durationInMinutes) {
    ...
    try {
        ...
        if (durationInMinutes !== undefined && durationInMinutes > 0) {
            ...
            await sock.sendMessage(chatId, { text: `༆Le group a été férmé jusqu'a༆ ${durationInMinutes} minutes.` }, { quoted: message });
            
            // Set timeout to unmute after duration
            setTimeout(async () => {
                try {
                    await sock.groupSettingUpdate(chatId, 'not_announcement');
                    await sock.sendMessage(chatId, { text: 'Dev shadow tech been unmuted✅.' });
                } catch (unmuteError) {
                    console.error('Error unmuting group:', unmuteError);
                }
            }, durationInMilliseconds);
        } else {
            await sock.sendMessage(chatId, { text: 'Dev shadow tech been muted✅.' }, { quoted: message });
        }
    } catch (error) {
